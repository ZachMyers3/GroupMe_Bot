...
parser.add_argument(
    "-c",
    "--config",
    help="ini file containing keys and other bot info",
    type=str,
    required=True,
)
parser.add_argument(
    "-g",
    "--groups",
    help="json file with the groups the bot will listen to",
    type=str,
    required=True,
)
...
def main():
    """
    :return: sets up the groups the bot will listen and
    then initialize the threads
    """
    ...
    try:
        ...
        print(
            "{} - Need groupme api key to continue".format(
    ...
    try:
        ...
        with open(os.path.abspath(group_file_path), "w") as finalized_groups:
            json.dump(
                chosen_groups, finalized_groups, sort_keys=True, indent=2
            )
    ...
    for group in groups:
        ...
        try:
            if group["enabled"]:
                ...
                print(
                    "{} - creating thread for {}".format(
                        get_current_datetime(), group.name
                    )
                )
                threading.Thread(
                    target=consume,
                    name=group.name,
                    daemon=True,
                    args=(bot, consume_time),
                ).start()
        except (
            IndexError,
            groupy.exceptions.BadResponse,
            requests.exceptions.HTTPError,
        ):
            print(
                "{} - '{}' group doesn't exist".format(
...
def consume(bot, seconds=1):
    ...
    :param seconds: units in seconds that the parsing of
        messages will pause after each parsing
    :return: either will process the messages and if a command is present,
        it'll post in the group chat forever.
    ...
    print(
        "{} - {}: Stopping {} thread".format(
            get_current_datetime(), bot.group.name, thread.name
        )
    )
