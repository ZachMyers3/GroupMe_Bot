...
class Tags:
    ...
    def create_json(self, group_id):
        ...
        if not os.path.exists(tag_folder):
            ...
            print(
                "{} - {}: created: {}".format(
                    get_current_datetime(), self.group_name, tag_folder
                )
            )
        ...
        if not os.path.exists(tag_json):
            ...
            print(
                "{} - {}: created: {}".format(
                    get_current_datetime(), self.group_name, tag_json
                )
            )
    ...
    def load_tags(self):
        ...
        tag_json = os.path.abspath(
            self.bot_path + "/../tags/" + self.group_id + ".json"
        )
        print(
            "{} - {}: loading: {}".format(
                get_current_datetime(), self.group_name, tag_json
    ...
    def reload_tags(self):
        ...
        """
        print(
            "{} - {}: reloading tags of {}".format(
                get_current_datetime(), self.group_name, self.group_name
            )
        )
        self.tags = self.load_tags()
    ...
    def parse_commands(self, message, owner, mentions):
        ...
        if command == "create":
            ...
            try:
                new_owner = list(
                    filter(lambda x: x.type == "mentions", mentions)
                )[0].user_ids[0]
    ...
    def get_tag_content(message, mentions):
        ...
        if mentions:
            url = list(
                filter(
                    lambda x: x.type == "image" or x.type == "video", mentions
                )
            )
            return url[0].url
    ...
    def post_tag(self, name):
        ...
        """
        print(
            "{} - {}: searching for tag: {}".format(
                get_current_datetime(), self.group_name, name
            )
        )
        if name in self.tags["tags"]:
    ...
    def list_tags(self):
        ...
        if len(all_tags) > 1000:
            all_tags = list(
                map(
                    "".join,
                    zip_longest(*[iter(all_tags)] * 1000, fillvalue=""),
                )
            )
            return all_tags
    ...
    def rename_tag(self, old_name, new_name, owner):
        ...
        """
        print(
            "{} - {}: Renaming tag '{}' to '{}'".format(
                get_current_datetime(), self.group_name, old_name, new_name
            )
        )
        if not self.tags["tags"]:
            return "There are no tags"
        if new_name in self.tags["tags"]:
            return (
                'The tag "{}" already exists. Can\'t change name to it'.format(
                    new_name
                )
            )
        try:
            if self.tags["tags"][old_name]["owner"] == owner:
                ...
                self.save_tags()
                return 'The tag "{}" has been renamed to "{}"'.format(
                    old_name, new_name
                )
    ...
    def gift_tag(self, name, owner, new_owner):
        ...
        try:
            if self.tags["tags"][name]["owner"] == owner:
                ...
                return 'The tag "{}" has been gifted to "{}"'.format(
                    name, new_owner
                )
            return 'You are not the owner of the tag "{}"'.format(name)
        except KeyError:
    ...
    def filter_tags(self, keyword):
        ...
        """
        filtered_tags = list(
            filter(lambda x: keyword in x, self.tags["tags"].keys())
        )
        ...
        if len(filtered_tags) > 1000:
            tag_names = list(
                map(
                    "".join,
                    zip_longest(*[iter(filtered_tags)] * 1000, fillvalue=""),
                )
            )
            return tag_names
    ...
    def find_owner_name(self, user_id):
        ...
        """
        try:
            return list(filter(lambda x: x.user_id == user_id, self.members))[
                0
            ].nickname
        except IndexError:
