"""Bot object each group will have that handle checking for commands
    and processing them"""
...
class Bot:
    ...
    def __init__(
        ...
        :param yt_key: youtube api key. need it to use yt_search but not
            needed for other commands
        :param delim: the first character that will let the bot know it is
            a command. default is "$"
        """
        self.group = group
        self.delim = delim
    ...
    def get_message(self):
        """
        :return: returns the latest message from a group. if there is an error,
            return None
        """
        try:
    ...
    def reload_group(self, stop=Event()):
        """
        :param stop: threading Event. not set by default so this method would
            be called every 10 minutes
        :return: updates the group name, group id, and group members of a
            group every 10 minutes
        """
        self.group.refresh_from_server()
        # print("{} - {}: Members of {}: {}".format(get_current_datetime(),
        # self.group.name, self.group.name, self.group.members))
    ...
    def find_owner_name(self, user_id):
        ...
        """
        return list(
            filter(lambda x: x.user_id == user_id, self.group.members)
        )[0]

    def find_avatar(self, message, mentions):
        """
        :param message: the avatar command. checks to see if it's a help call
            or actual usage
        :param mentions: list of attachments with the message. uses it to
            check for mentions
        :return: avatar url of person mentioned or an error message saying to
            mention the user
        """
        if message[2] == "help":
    ...
    def process_message(self, message):
        """
        :param message: checks if the message is a valid comand and execute
        ...
        if message is not None:
            try:
                ...
                if delim == self.delim and command in self.valid_commands:
                    ...
                    if command == "avatar":
                        result = self.find_avatar(
                            message_text, message.attachments
                        )
                    if command == "git":
                        result = self.ult.git()
                    if command == "yt":
